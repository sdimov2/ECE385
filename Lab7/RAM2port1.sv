...
module byte_enabled_true_dual_port_ram
	#(
		...
		ADDRESS_WIDTH = 6, // This we will have to change  -Owen
		BYTES = 4, // 32 byte width is good
		DATA_WIDTH_R = BYTE_WIDTH * BYTES // This is correct
)
(
	...
	// localparam RAM_DEPTH = 1 << ADDRESS_WIDTH; // Originally uses BIT X as the depth
	localparam RAM_DEPTH = 64;

	// model the RAM with two dimensional packed array
	logic [BYTES-1:0][BYTE_WIDTH-1:0] ram[0:RAM_DEPTH-1];

	wire [DATA_WIDTH_R-1:0] data_wire1; // Change these to wires to make 
	wire [DATA_WIDTH_R-1:0] data_wire2;
	...
	always_comb
	begin
		data_wire1 = ram[addr1];
	end

	assign data_out1 = data_wire1;
	...
	always_comb 
	begin : blockName
		data_wire2 = ram[addr2];
	end

	assign data_out2 = data_wire2;
