...
void printSignedHex0(signed char value) {
	BYTE tens = 0;
	BYTE ones = 0;
	WORD pio_val = IORD_ALTERA_AVALON_PIO_DATA(HEX_DIGITS_BASE);
	if (value < 0) {
		setLED(11);
		value = -value;
	} else {
...
void printSignedHex1(signed char value) {
	BYTE tens = 0;
	BYTE ones = 0;
	DWORD pio_val = IORD_ALTERA_AVALON_PIO_DATA(HEX_DIGITS_BASE);
	if (value < 0) {
		setLED(10);
		value = -value;
	} else {
...
int main() {
	...
	while (1) {
		...
		if (GetUsbTaskState() == USB_STATE_RUNNING) {
			...
			else if (device == RAPHNET) {
				//printf("Sick you made it to raphnet poll.\n");
				rcode = raphnetPoll(&raphnet_buf);
				if (rcode == hrNAK)
					continue;

				if (buttons != raphnet_buf.buttons){
