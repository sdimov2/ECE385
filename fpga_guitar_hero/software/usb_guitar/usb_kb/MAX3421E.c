...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE rxdata;	//don't think this is used, but give it a non-null pointer anyway
	int return_code;
	return_code = alt_avalon_spi_command(SPI_0_BASE,0,
						1, &regdir,
						0, &rxdata,
						ALT_AVALON_SPI_COMMAND_MERGE);	//issue command, don't deselect slave.
	return_code = alt_avalon_spi_command(SPI_0_BASE,0,
						1, &val,
						0, &rxdata,
						0);	//issue data, deselect at the end.

	if (return_code < 0)
		printf("SPI Write error! code: %d\n", return_code);
}
//multiple-byte write
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	return_code = alt_avalon_spi_command(SPI_0_BASE, 0,
						nbytes, data,
						0, &rxdata,
						0);	//send data, don't deselect

	if (return_code < 0)
		printf("SPI Write Multiple error! code: %d\n", return_code);
